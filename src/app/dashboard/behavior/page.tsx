    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl p-8 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          <Button className="bg-white text-indigo-600 hover:bg-indigo-50 px-6 py-6 text-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <Card className="border-l-4 border-l-gray-500 hover:shadow-xl transition-all duration-300">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium text-gray-600">
              Neutral
            </CardTitle>
                  ...
                  {behaviors.map((behavior) => (
                    <tr key={behavior.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="flex items-center">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-500 text-white flex items-center justify-center font-semibold mr-3">
                            {behavior.student.name.charAt(0)}{behavior.student.surname.charAt(0)}
                          </div>
                          <div>
                            <Link 
                            ...
                            <p className="text-sm text-gray-500">
                              {behavior.student.class.name} • Grade {behavior.student.grade.level}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="p-4">
                        ...
                        >
                          {behavior.type === "POSITIVE" && <CheckCircle className="w-3 h-3 mr-1" />}
                          {behavior.type === "NEGATIVE" && <AlertCircle className="w-3 h-3 mr-1" />}
                          {behavior.type}
                        </span>
                      </td>
                      <td className="p-4">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm text-gray-600">• {behavior.title}</span>
                        </div>
                        <p className="text-sm text-gray-700 mb-2">
                          <span className="font-semibold">Action Taken:</span> {behavior.actionTaken}
                        </p>
                        <p className="text-xs text-gray-500">
                          {format(new Date(behavior.date), "MMM dd, yyyy")} • By {behavior.reportedBy}
                        </p>
                      </div>
                    </div>
                  </div>
